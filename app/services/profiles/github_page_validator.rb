...
module Profiles
  ...
  class GithubPageValidator
    ...
    def check_page_title
      title = browser.at_css(GithubSelectors::TITLE_TAG)&.text&.downcase || ""
    ...
    def check_profile_elements
      html = browser.body
      has_profile = GithubSelectors::PROFILE_ELEMENT_MARKERS.any? { |marker| html.include?(marker) }
      return if has_profile

      if GithubSelectors::ERROR_PATTERNS.any? { |pattern| html.match?(pattern) }
        raise ProfileNotFoundError, "Perfil não encontrado"
      end
    end
  end
